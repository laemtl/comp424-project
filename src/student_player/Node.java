...
public class Node<A> {
    ...
    public <B> B reduce(BiFunction<A, List<B>, B> func) {
    	return func.apply(value, getChildren()
    			.stream()
    			.map(node -> node.reduce(func))
    			.collect(Collectors.toList()));
    }

    public static <A, B> Node<B> expand(Function<A, Pair<B, List<A>>> func, A a) {
    	Pair<B, List<A>> pair = func.apply(a);
    	Supplier<List<Node<B>>> children = () -> pair.right
    			.stream()
    			.map((a2) -> Node.expand(func, a2))
    			.collect(Collectors.toList());
    	return new Node<B>(pair.left, children);
    }
    
    public Node<A> truncateAtDepth(int n) {
    	Supplier<List<Node<A>>> supplier = () -> n <= 1 ? 
    			  Arrays.asList()
    			: getChildren()
    				.stream()
    				.map(child -> child.truncateAtDepth(n - 1))
    				.collect(Collectors.toList());
